...
Uses a lightweight LLM enhancer to minimize memory usage.
"""
...
# Import lite LLM enhancer
from core.lite_llm_enhancer import get_lite_llm_enhancer
...
@st.cache_resource
def initialize_llm():
    """Initialize the Lite LLM (cached to avoid reloading)."""
    return get_lite_llm_enhancer()
...
def process_creation(prompt, llm_enhancer):
    ...
    try:
        with st.spinner("Enhancing prompt with rule-based system..."):
...
def main():
    st.set_page_config(
        page_title="Creative AI Pipeline (Low Memory)",
        page_icon="🧠",
        layout="wide",
    )
    
    st.title("🌟 Creative AI Pipeline (Low Memory Version)")
    st.subheader("Turn your text prompts into images and 3D models")
    
    # Initialize the LLM (cached)
    with st.spinner("Loading Lite LLM..."):
    ...
    st.sidebar.info(
        ...
        with a low-memory rule-based prompt enhancer.
    ...
    st.sidebar.info(
        """
        **Mode**: Low Memory Rule-Based System
        
        This version uses a lightweight rule-based approach
        instead of loading a large language model.
